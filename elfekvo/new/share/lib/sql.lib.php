...
function modulGetRecord($config, $p_itemValues)
	...
	if ($config["modulSelect"]!="core_users")
		...
		$getRecordSQL .= "(SELECT core_users.nickname FROM core_users WHERE core_users.id = ".$config[$config["modulSelect"]]["mainTable"].".insertUserID ) AS insertWho,";
		$getRecordSQL .= "(SELECT core_users.nickname FROM core_users WHERE core_users.id = ".$config[$config["modulSelect"]]["mainTable"].".modifyUserID ) AS modifyWho";
	...
	if (!empty($config[$config["modulSelect"]]["sqlKey"]) || !empty($_SESSION[$config["modulSelect"]["whichSystemAndMode"].$config["modulSelect"]."sqlKey"]))
		...
		$p_itemValues["x_insertWho"] = $config[$config["modulSelect"]]["resultRow"]["insertUserID"];
		$p_itemValues["x_modifyWho"] = $config[$config["modulSelect"]]["resultRow"]["modifyUserID"];
		}
	else
		{
//		jumpToPage($config[$config["modulSelect"]]["whichBack"]);
...
function coreUsersConfigClear($config,$configField,$configValue)
	{
	$deleteSQL = "DELETE FROM core_users_config WHERE fieldName='".$configField."' AND core_usersID='".$_SESSION[$config["configShare"]["whichSystemAndMode"] . "status_UserID"]."'";
	$config["dbConnect"]->exec($deleteSQL);
	}

function coreUsersConfigSet($config,$configField,$configValue)
	{
	$searchSQL=$config["dbConnect"]->prepare("SELECT id FROM `core_users_config` WHERE active=1 AND fieldName='".$configField."' AND core_usersID='".$_SESSION[$config["configShare"]["whichSystemAndMode"] . "status_UserID"]."'");
	$searchSQL->execute();
	$searchCount = $searchSQL->rowCount();
	$searchSQL->closeCursor();
	if ($searchCount > 0)
		{
		$updateSQL = "UPDATE core_users_config SET fieldName='".$configField."',fieldValue='".$configValue."',modifyWhen='".db_actual_datetime()."' WHERE active=1 AND fieldName='".$configField."' AND core_usersID='".$_SESSION[$config["configShare"]["whichSystemAndMode"] . "status_UserID"]."'";
		$config["dbConnect"]->exec($updateSQL);
		}
	else
		{
		$insertSQL="INSERT INTO core_users_config (fieldName,fieldValue,active,core_usersID,insertWhen) VALUES ";
		$insertSQL .= "('".$configField."', '".$configValue."', '1', '".$_SESSION[$config["configShare"]["whichSystemAndMode"] . "status_UserID"]."', '".db_actual_datetime()."')";
